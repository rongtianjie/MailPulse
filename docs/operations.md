...
当前报告行动项功能新增 `report_action_states` 表。应用启动或 `init-db` 会为现有数据库创建该缺失表，不需要重置数据库；只有发生无法由 `create_all` 补齐的既有表结构变更时，开发环境才需要执行 `reset-db --confirm`。生产数据结构升级仍应先制定备份和迁移方案，不能直接使用重置命令。
...
- HTML 邮件显示异常：确认邮件客户端支持 `multipart/alternative` 和 HTML 邮件；不支持时应自动显示其中的 Markdown 纯文本版本。不同邮件客户端可能对内嵌 CSS 的支持程度不同，但不影响报告内容。
- 行动项状态保存失败：确认登录 Session 仍有效、页面未长时间停留导致 CSRF 状态失效，并检查网页服务日志。行动项写入要求报告属于当前用户，且序号存在于报告结构化摘要中。
- 报告时间不符合预期：检查任务基本信息中的 IANA 时区。默认 `Asia/Shanghai` 不额外显示时区名称，非默认时区会在报告时间后标注。
