...
- 报告 Markdown 安全渲染、网页查看（始终开启）、HTML/纯文本 SMTP 投递、失败重试和运行记录。
- 用户账号设置支持修改密码与显示名称。
...
新建任务采用四步向导：基本信息 → 收件邮箱（IMAP 配置与即时连接验证；SMTP 为可选折叠项，用于邮件投递）→ 筛选规则（表单化编辑：字段/操作符/值，规则列表按优先级排列、可上移下移）→ 投递渠道（网页内直接查看始终开启；邮件投递地址支持添加、编辑、删除、启停与发送测试邮件）。基本信息中的时区使用常用 IANA 时区下拉框；已有的非预置 IANA 时区值在编辑时仍会保留。
...
- 定时运行：每日、每周或五段 Cron 表达式，配合任务 IANA 时区和邮件回看时间，由后台 worker 自动触发。
...
模型先生成结构化 JSON，服务端再确定性地生成 Markdown 报告。网页将 Markdown 转换为经过标签、属性和 URL 协议白名单清洗的 HTML；SMTP 邮件使用 `multipart/alternative`，同时提供 Markdown 纯文本版本和安全 HTML 版本，以兼容不同邮件客户端。

新报告标题使用“邮件归纳｜筛选时间段｜关键词”的形式。时间段取实际筛选窗口并按任务时区显示；关键词只从启用规则的正向文本条件中提取，忽略否定、正则、日期和数值比较条件。没有可提取关键词时只保留时间段。历史报告标题不会被追溯修改。

报告详情页支持勾选行动项。完成状态保存在独立数据记录中，不改写原始报告 Markdown；发送或重试 SMTP 邮件时会读取当时最新的完成状态并生成邮件快照。报告详情页还支持手动发送和失败重试，并保存投递状态、尝试次数和错误信息。

报告详情、报告列表、任务历史和首页最近报告均按对应任务时区显示报告时间。默认 `Asia/Shanghai` 不额外显示时区名称；其他时区会在时间段后标注 IANA 时区。
