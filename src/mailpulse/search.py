...
class SearchService:
    ...
    def index_message(self, message: CanonicalMessage) -> None:
        ...
        self.session.execute(
            text(
                "INSERT INTO message_search(message_id, owner_user_id, subject, sender, body_text) "
        ...
        if query and self._fts_available() and len(query) >= 3:
            rows = self.session.execute(
                text(
                    ...
                    "WHERE owner_user_id = :owner_id AND message_search MATCH :query LIMIT :limit"
                ),
